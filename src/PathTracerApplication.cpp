...
void PathTracingDemo::Run()
{
	...
	while (!ApplicationWindow.ShouldClose())
	{
		...
		glm::vec2 SampleMultiplier = AllowPathTrace && camera3d->IsCameraMovedf() ? glm::vec2(0.125f,0.125f) : glm::vec2(1.0f, 1.0f);

		FUSIONCORE::DrawTextureOnQuad(ViewportImage, { 0,0 }, { mode.width,mode.height }, *camera3d, *Shaders->TextureOnQuadShader, SampleMultiplier, 1.7f, 1.6f);
