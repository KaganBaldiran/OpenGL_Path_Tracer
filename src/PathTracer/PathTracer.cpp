...
void FUSIONCORE::PathTracer::Render(Window& window,Shader& shader,Camera3D& camera,CubeMap* Cubemap,unsigned int DenoiseSampleCount)
{
	...
	bool isTracingPipelineAltered = ShouldRestart || camera.IsCameraMovedf() || window.IsWindowResizedf() || shader.IsRecompiled() ||
		             PreviouslyUsedCubeMap != Cubemap || PreviousImportedHDRIcount != Cubemap->GetImportedHDRICount();

	bool isDummySample = false;

	if (isTracingPipelineAltered)
	{
	...
	if (ProgressiveRenderedFrameCount < TargetSampleCount)
	{
		...
		shader.setVec2("WindowSize", camera.IsCameraMovedf() ? WindowSize * 0.125f : WindowSize);
		...
		{
			numGroupsX = (WindowSize.x * 0.125f + workGroupSizeX - 1) / workGroupSizeX;
			numGroupsY = (WindowSize.y * 0.125f + workGroupSizeY - 1) / workGroupSizeY;
		}
		else
		{
			numGroupsX = (WindowSize.x + workGroupSizeX - 1) / workGroupSizeX;
			numGroupsY = (WindowSize.y + workGroupSizeY - 1) / workGroupSizeY;
		}
