                           ...
                           in float Sheen,in float SheenTint,in float ClearCoat,in float ClearCoatRoughness,in float IOR,inout float LightPDF)
{
     ...
     if(LdotLnormal <= 0.0f) return vec3(0.0f);
     
     float Distance = length(SampledPosition - Position);
     vec3 Radiance = EmissiveBoost * (Emissive.xyz);
     LightPDF = 1.0f / (EmissiveLightData.y * EmissiveTriangleCount);
     float GeometryTerm = (LdotLnormal * NdotL) / (Distance * Distance);

     return ShadowColor * mix(0.0f,1.0f,IsVisible) * GeometryTerm * Eval(Sample,Radiance,Direction,L,Normal,Roughness,Metallic,Albedo,Alpha,Sheen,SheenTint,ClearCoat,ClearCoatRoughness,IOR,false);
}
...
vec4 TraceRay(in vec3 rayOrigin,in vec3 rayDirection,in int SampleCount,inout vec3 FirstHitNormal,inout vec3 FirstHitAlbedo,inout vec3 FirstHitPosition)
{
    ...
    { 
        ...
        if(length(data.Emissive.xyz) > 0.0f)
        {
            ...
            if(PreviousHitRoughness <= 1e-3)
            {
               Albedo.xyz += Throughput * EmissiveBoost * (data.Emissive.xyz) * PowerHeuristic(BSDFpdf,lightPdf);  
            }
            break;
        }
        ...
        if(data.Roughness > 1e-3)
        {
           ...
           if(length(EmissiveLightContribution) > 0.0f) Albedo.xyz += Throughput * vec3(EmissiveLightContribution) / lightPdf;
        }
        ...
        PreviousHitRoughness = data.Roughness;
        BSDFpdf = Sample.pdf;
    }
